 ...
 *   - "After a vowel, only `L` absorbs the schwa, and not even `L` after
 *     `IY`" is re-applied here against ARPABET's fixed vowel-symbol set,
 *     since a Rhyme Key strips every stress digit and so carries nothing
 *     `isVowel` (phonology.ts) could read.
 *
 ...
 *
 * `SCHWA`, `SYLLABIFIABLE_SONORANTS`, `VOWEL_ABSORBING_SONORANT` and
 * `UNABSORBING_VOWEL` restate four literals `normalise.ts`'s `syllabicConsonant`
...
/** True if a vowel immediately before the schwa blocks this pair from twinning. */
function blockedByPrecedingVowel(preceding: string, sonorant: string): boolean {
  return (
    isVowelSound(preceding) && (sonorant !== VOWEL_ABSORBING_SONORANT || preceding === UNABSORBING_VOWEL)
  );
}
...
export function schwaTwinOf(key: RhymeKey): RhymeKey | null {
  ...
  if (beforeSonorant === SCHWA && parts.length >= 3) {
    ...
    if (blockedByPrecedingVowel(preceding, sonorant)) return null;
