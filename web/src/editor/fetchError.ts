 ...
 * disagreement posts to the supplement-candidate endpoint (#163) — the path a
 * player's Appeal also reaches — because the record it writes is a Candidate
 * and that queue already exists, the point of that slice being to introduce no
 * endpoint of its own. It shares these helpers anyway, and for the reason they
 * were extracted rather than by coincidence: that endpoint refuses in the same
 * `{ error: string }` shape, from `src/supplementCandidate.ts`'s own sentence.
 *
 ...
 * it — a day, a Tier verdict, a demotion, an add, the status read or a
 * Candidate.
 *
 ...
 * ${response.status}.\`` — which was five copies of the same three lines when
 * it was extracted (#162), one per hook, and is six now that the disagreement
 * recorder is written against it. It stops there rather than reaching further up
 ...
 * point the function is not shorter than the call sites it replaced, only more
 * indirect than they were.
 *
 ...
 * from all five: a broken connection there may have written part of the
 ...
 * the six hooks then does the same thing with: a value to use, or a sentence
